...
public class Sector extends DashboardModel {
	...
	public boolean createSector(String sectorName) {
		...
		try (Connection conn = connector(); PreparedStatement pstmt = conn.prepareStatement(sqlexists)) {
		...
		if (!exists) {
			...
			try (Connection conn = connector(); PreparedStatement pstmt = conn.prepareStatement(sqlupdate)) {
	...
	public void deleteSector(String sectorName) {
		...
		try (Connection conn = connector(); PreparedStatement pstmt = conn.prepareStatement(delete)) {
			pstmt.setString(1, sectorName);
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void archiveSector(String sectorName) {
		...
		try (Connection conn = connector(); PreparedStatement pstmt = conn.prepareStatement(sqlarchived)) {
			pstmt.setString(1, "");
	...
	public void unarchiveSector(String sectorName) {
		...
		try (Connection conn = connector(); PreparedStatement pstmt = conn.prepareStatement(sqlunarchived)) {
			pstmt.setString(1, "active");
			pstmt.setString(2, "");
	...
	// String text = "I will come and meet you at the woods 123woods and all the
	// woods";
	//
	...
	// System.out.println("I will come and meet you at the
	// 123woods".matches(".*\\badded\\b.*"));
